...
module.exports = (function (app) {
	...
	/**
	 * Initializes routes, effectively binding all resources for the
	 * application.
	 **/
	...
	app.start = function (callback) {
		...
		async.auto({
			config : loadConfig,
			data : ['config', 'logging', async.apply(data.initialize, app)],
			logging : ['config', createLogger],
			routes : ['config', 'logging', 'data', initializeRoutes],
			server : ['config', 'logging', 'routes', createServer]
		}, function (err) {
			if (err) {
