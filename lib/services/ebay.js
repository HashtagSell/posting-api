var
	...
	DEFAULT_COMPATIBILITY_LEVEL = '907',
...
module.exports = function (app, self) {
	...
	function getSuggestedCategoriesXML (query) {
		return util.format(
	...
	self.create = function (posting, ebayCategory, siteId, callback) {
		var
			...
			startTime = new Date(),
			xml = getAddFixedPriceItemXML(posting, undefined); //ebayCategory);
	...
	self.revise = function (itemId, posting, ebayCategory, siteId, callback) {
		var
			...
			parser = new xml2js.Parser(),
			startTime = new Date(),
			xml = getReviseFixedPriceItemXML(itemId, posting, undefined); //ebayCategory);
	...
	self.suggestedCategories = function (query, siteId, callback) {
		if (typeof callback === 'undefined' && typeof siteId === 'function') {
			callback = siteId;
			siteId = DEFAULT_SITE_ID;
		}
		...
		var
			...
			parser = new xml2js.Parser(),
			startTime = new Date(),
			xml = getSuggestedCategoriesXML(query);
