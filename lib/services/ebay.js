var
	...
	DEFAULT_COMPATIBILITY_LEVEL = '927',
...
module.exports = function (app, self) {
	...
	var
		...
		xmlEnvelope = {
			...
			setNotificationPreferences : [
				...
				/*
				// possibily consider sending additional emails to a HashtagSell account
				...
				// when a question about the item is asked via Ebay
				'		<NotificationEnable>',
				'			<EventType>AskSellerQuestion</EventType>',
				'			<EventEnable>Enable</EventEnable>',
				'		</NotificationEnable>',
				// once item is bought and paid for via Ebay
				'		<NotificationEnable>',
				'			<EventType>AuctionCheckoutComplete</EventType>',
				'			<EventEnable>Enable</EventEnable>',
				'		</NotificationEnable>',
				// when an item recieves a best offer via Ebay
				'		<NotificationEnable>',
				'			<EventType>BestOffer</EventType>',
				'			<EventEnable>Enable</EventEnable>',
				'		</NotificationEnable>',
				// when an item expires on Ebay with no buyer
				'		<NotificationEnable>',
				'			<EventType>EndOfAuction</EventType>',
				'			<EventEnable>Enable</EventEnable>',
				'		</NotificationEnable>',
				// when feedback is received from an Ebay user regarding a posting
				'		<NotificationEnable>',
				'			<EventType>Feedback</EventType>',
				'			<EventEnable>Enable</EventEnable>',
				'		</NotificationEnable>',
				// when an item is marked as sold for single-quantity, fixed price listing
				'		<NotificationEnable>',
				...
				'		<eBayAuthToken>%s</eBayAuthToken>',
				'	</RequesterCredentials>',
				'</SetNotificationPreferencesRequest>'].join(os.EOL),
			suggestedCategories : [
				'<?xml version="1.0" encoding="utf-8"?>',
	...
	function getAddFixedPriceItemXML (posting, ebayCategory, paypalEmail, token) {
		...
		ebayPosting.Description = posting.body.replace('@', 'at ').replace('#', '');
	...
	function getSetNotificationsPreferencesXML (token) {
		return util.format(
			xmlEnvelope.setNotificationPreferences,
			token || config.token);
	}
	...
	self.setNotificationPreferences = function (siteId, token, callback) {
		if (typeof callback === 'undefined' && typeof token === 'function') {
			callback = token;
			token = siteId;
			siteId = undefined;
		}

		if (typeof token === 'undefined' && typeof siteId === 'function') {
			callback = siteId;
			siteId = undefined;
		}
		...
		var
			headers = getHeaders('SetNotificationPreferences', siteId),
			parser = new xml2js.Parser(),
			startTime = new Date(),
			xml = getSetNotificationsPreferencesXML(token);
		...
		request({
			...
			app.log.trace(
				'request to set notification preferences with eBay completed in %s',
				countdown(startTime, new Date(), countdown.MILLISECONDS).toString());
			...
			parser.parseString(body, function (err, result) {
				...
				err = err || getResponseError(result);
				if (err) {
					app.log.warn('unable to set notification preferences');
