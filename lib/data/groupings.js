var
	...
	ebayCategoriesSchema = mongoose.Schema({
		code : {
			index : {
				unique : true
			},
...
module.exports = function (app, self) {
	...
	extensions.toObject(ebayCategoriesSchema);
	extensions.toObject(groupingsSchema);

	// create mongoose model
	var
		EbayCategory = mongoose.model('ebaycategories', ebayCategoriesSchema),
		Grouping = mongoose.model('groupings', groupingsSchema);
	...
	self.find = function (options, callback) {
		...
		Grouping
			...
			.page(options, function (err, groupings) {
				...
				return callback(null, extensions.transformPageResults(groupings));
			});
	};
	...
	self.findEbayCategory = function (code, callback) {
		var verr;

		EbayCategory
			.findOne({ code : code })
			.exec(function (err, ebayCategory) {
				if (err) {
					verr = new VError(
						err,
						'findEbayCategory for code %s failed',
						code);
				...
				if (!ebayCategory) {
					app.log.trace(
					'no Ebay category match found for code %s',
					 code);

					return callback();
				}

				return callback(null, ebayCategory.toObject({ transform : true }));
			});
	};
