...
module.exports = function (app, self) {
	...
	self.findByReviewId = function (reviewId, includeDeleted, callback) {
		// verify case when includeDeleted is not supplied
		if (typeof callback === 'undefined' &&
			typeof includeDeleted === 'function') {
	...
	self.remove = function (reviewId, callback) {
		...
		Review
			.findOne({ deleted : false, reviewId : reviewId })
			.exec(function (err, review) {
				if (err) {
		...
		Review
			...
			.exec(function (err, upsertReview) {
				...
				if (!upsertReview) {
					...
					// re-enable the offer
					if (upsertReview.deleted && typeof review.deleted === 'undefined') {
						app.log.trace(
							're-enabling existing review with reviewId %s',
							reviewId);

						upsertReview.deleted = false;
					}
				}
