...
module.exports = function (app, self) {
	...
	self.findByOfferId = function (offerId, includeDeleted, callback) {
		...
		Offer
			...
			.exec(function (err, offer) {
				...
				return callback(null, offer.toObject());
			});
	};
	...
	self.findByUserId = function (userId, options, callback) {
		var verr;
		...
		Offer
			...
			.page(options, function (err, offers) {
				if (err) {
					verr = new VError(err, 'unable to find offers for user %s', userId);
					verr.options = options;
		...
		Offer
			...
			.exec(function (err, upsertOffer) {
				...
				upsertOffer.save(function (err) {
					...
					// return to caller
					return callback(null, upsertOffer.toObject());
				});
			});
	};
