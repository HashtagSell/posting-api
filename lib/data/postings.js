...
module.exports = function (app, es, self) {
	...
	self.aggregateByCategory = function (searchTerms, exclusiveAnd, callback) {
		if (typeof callback === 'undefined' && typeof exclusiveAnd === 'function') {
			callback = exclusiveAnd;
			exclusiveAnd = false;
		}
		...
		// the DEFAULT_GROUPING_HEADING_TYPE contains CRAIG listings
		// the DEFAULT_POSTING_TYPE does not contain CRAIG listings and has a TTL
		es.search({
				_type : DEFAULT_GROUPING_HEADING_TYPE, //DEFAULT_POSTING_TYPE,
				size : 0
			},
			query,
			function (err, result) {
				...
				if (result.aggregations && result.aggregations['group_by_categoryCode']) {
					/* jshint sub : true */
					var buckets = result.aggregations['group_by_categoryCode'].buckets;
					...
					buckets.forEach(function (code) {
						...
						if (code.relevance > 0 || code.count > 0) {
				...
				function (hits, next) {
					...
					Posting
						...
						.exec(function (err, postings) {
							if (err) {
								verr = new VError(err, 'unable to find posting details');
