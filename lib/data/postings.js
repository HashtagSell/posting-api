...
module.exports = function (app, self) {
	...
	self.aggregateByCategory = function (searchTerms, callback) {
		...
		try {
			searchRegex = new RegExp(searchTerms, 'gim');
		} catch (ex) {
			verr = new VError(ex, 'unable to create RegExp from search terms');
			verr.searchTerms = searchTerms;

			return setImmediate(callback, verr);
		}
