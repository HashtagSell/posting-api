...
module.exports = function (app, self) {
	...
	self.findById = function (externalId, source, callback) {
		...
		if (typeof callback === 'undefined' && typeof source === 'function') {
			return self.findByPostingId(externalId, source);
		}
		...
		Posting
			...
			.exec(function (err, posting) {
				...
				return callback(null, posting.toObject());
			});
	};
	...
	self.findByPostingId = function (postingId, callback) {
		...
		Posting
			...
			.exec(function (err, posting) {
				if (err) {
					verr = new VError(err, 'findById for posting %s failed', postingId);
					return callback(verr);
				}
