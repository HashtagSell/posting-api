var
	...
	ES_SUPPORTED_FIELDS_REGEX = /heading|body|categoryCode|username/gi,
...
module.exports = function (app, es, self) {
	...
	function search (options, callback) {
		var
			findKeywords = function (filters, keywords) {
				Object.keys(filters).forEach(function (filterType) {
					...
					if (ES_SUPPORTED_KEYWORDS.indexOf(filterType) > 0) {
						Object.keys(filters[filterType]).forEach(function (field) {
							...
							if (ES_SUPPORTED_FIELDS_REGEX.test(field)) {

								// push keyword to the keywords list
								keywords.push(filters[filterType][field]);
		...
		if (returnOptions.filters && returnOptions.filters.mandatory) {
			findKeywords(returnOptions.filters.mandatory, mandatory);
		}

		/*
		// NOTE: optional not currently supported...
		if (options.filters && options.filters.optional) {
			findKeywords(options.filters.optional, optional);
		}
		//*/
