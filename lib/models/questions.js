...
var
	...
	SOCKET_EVENT_DELETE_QUESTION = 'delete-question',
		...
		async.waterfall([
				...
				function (removedQuestion, done) {
					data.postings.findById(postingId, function (err, posting) {
						...
						return done(err, removedQuestion, posting);
					});
				},
			], function (err, removedQuestion, posting) {
				if (err && !err.statusCode) {
					modelError = new errors.PersistenceError(
						err,
				...
				services.realtime.emit(
					SOCKET_CHANNEL,
					SOCKET_EVENT_DELETE_QUESTION,
					{
						posting : posting,
						questionId : questionId,
						username : removedQuestion.username,
						timestamp : new Date()
					});

				return callback(null, removedQuestion);
			});
	};
