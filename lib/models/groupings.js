...
module.exports = function (app, data, services, self) {
	...
	self.findPopular = function (searchTerms, callback) {
		var
			keywords = data.tokenizer.analyzeKeywords(searchTerms),
			modelError,
			startTime = new Date();
		...
		async.waterfall([
			...
			// fallbuck to 'or' if no results found
				...
				data.postings.aggregateByCategory(
					...
					function (err, result) {
						if (err) {
							modelError = new errors.PersistenceError(
								err,
								'unable to find (without exclusive and) popular categories');
							modelError.keywords = keywords;
							modelError.searchTerms = searchTerms;
						...
						if (!result) {
							modelError = new errors.ResourceNotFoundError(
								'nothing exists matching specified (without exclusive and) search term(s)');
							modelError.keywords = keywords;
							modelError.searchTerms = searchTerms;
						...
						app.log.trace('findPopular (without exclusive and) completed in %s',
							countdown(startTime, new Date(), countdown.MILLISECONDS).toString());
