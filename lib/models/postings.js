...
module.exports = function (app, data, services, self) {
	...
	self.findMostRecentExternal = function (callback) {
		var
			modelError,
			startTime = new Date();

		data.postings.findMostRecentExternal(function (err, posting) {
			if (err) {
				modelError = new errors.PersistenceError(
					err,
	...
	self.publish = function (postingId, channels, callback) {
		...
		if (channels.ebay && typeof channels.ebay === 'boolean') {
			channels.ebay = {
				paypalEmail : app.config.ebay.paypalEmail,
				token : app.config.ebay.token
			};
		}
		...
		async.waterfall([
			...
			function (posting, done) {
				...
				app.log.trace('updating posting with facebook and/or twitter details');
	...
	self.upsert = function (postings, callback) {
		...
		// create each posting in the array
		return async.map(postings, upsertPosting, function (err, result) {
			if (err) {
				return callback(err);
			}
