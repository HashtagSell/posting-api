...
module.exports = function (app, data, services, self) {
	...
	function createPosting (posting, callback) {
		...
		data.postings.upsert(postingId, posting, function (err, newPosting) {
			...
			// fire and forget a request to create the posting with eBay
			// TODO : base this on input from the posting itself
			data.groupings.findEbayCategory(
				posting.categoryCode,
				function (err, ebayCategory) {
					if (err) {
						app.log.error(
							'error encountered while attempting to find eBay category');
						app.log.error(err);
					...
					if (ebayCategory && ebayCategory.ebay) {
						...
						app.log.trace(
							'sending posting to eBay in category %d and site %d',
							categoryId,
							siteId);
					}

					services.ebay.create(newPosting,
						categoryId,
						siteId,
						function (err, result) {
							// the results here...
