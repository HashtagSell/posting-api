...
module.exports = function (app, data, services, self) {
	...
	function mapPosting (originalPosting) {
		var posting = {
			...
			publishing : originalPosting.publishing || {}
		};
	 ...
	 **/
	self.find = function (req, callback) {
		var
			calculateDistance = false,
			modelError,
			options = req.queryOptions,
			clientIP = self.checkIfPrivateOrLocalIP(req.connection.remoteAddress),
			normalizeCategories =
		...
		if (!validation.isEmpty(options.geo) &&
		...
		} else if (!validation.isEmpty(options.geo) &&
			validation.isEmpty(options.geo.coords)) {
			//If long/lat coords are not provided then geolocate the IP

			var promise = new Promise(function (resolve, reject) {
				self.geolocateIP(clientIP, function (error, response) { //Use our geolocation service
					if (error) {

						reject(error);  //Geolocation failed
			...
			promise.then(function (locationData) {
				...
				options.geo = {};
				options.geo.coords = [locationData.longitude, locationData.latitude];
				options.geo.max = parseInt(options.geo.max, 10) || DEFAULT_MAX_DISTANCE;
				options.geo.min = parseInt(options.geo.min, 10) || DEFAULT_MIN_DISTANCE;
	...
	self.publish = function (postingId, channels, callback) {
		...
		async.waterfall([
			...
			foundPosting.publishing = foundPosting.publishing || {};
			foundPosting.publishing.ebay = result;

			// update the posting
			self.update(postingId, foundPosting, function (err) {
	 ...
	 **/
	self.checkIfPrivateOrLocalIP = function (clientIP) {
		var parts = clientIP.split('.');

		var dummyIP = "216.38.134.18";
	 ...
	 **/
	self.geolocateIP = function (clientIP, promise) {

		var freeGeoIp = "http://localhost:8080/json/" + clientIP;
