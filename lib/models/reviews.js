	...
	self.update = function (reviewId, review, callback) {
		...
		async.waterfall([
				...
				function (foundReview, done) {
					if (!foundReview.isExchanged && review.isExchanged) {
						removePosting = true;
					}
				...
				// if isExchanged, remove the posting from the search index
				function (updatedReview, done) {
					if (!removePosting) {
						return setImmediate(function () {
							return done(null, updatedReview);
						});
					}

					// remove the posting
					data.postings.remove(updatedReview.postingId, function (err) {
						if (err) {
							return done(err);
						}
