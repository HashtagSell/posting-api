...
module.exports = function (app, data, services, self) {
	...
	self.acceptOffer = function (postingId, offerId, acceptedProposal, callback) {
		...
		async.waterfall([
				...
				function (offer, done) {
					...
					//Don't notify the seller that they have update.. Notify them of acceptance!
					var suppressUpdateNotification = true;

					// update the offer to save the acceptedTime
					return self.update(postingId, offerId, offer, suppressUpdateNotification, done);
				},
		...
		async.waterfall([
				...
				function (offer, done) {
					...
					if (!isFound) {
						...
						var suppressUpdateNotification = false;
	 ...
	 **/
	self.update = function (postingId, offerId, offer, suppressUpdateNotification, callback) {
		var
			modelError,
			startTime = new Date();
		...
		async.waterfall([
				...
				//console.log('should we suppress update email???: ', suppressUpdateNotification);
				if(!suppressUpdateNotification) {

					app.log.trace('Queuing update email');
					...
					app.log.trace('Suppressing update email');
				}
