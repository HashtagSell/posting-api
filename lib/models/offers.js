...
module.exports = function (app, data, services, self) {
	 ...
	 **/
	self.findAcceptedProposalsForNotification = function (startDate, options, callback) {
		var
			modelError,
			startTime = new Date();
		...
		if (validation.isEmpty(startDate)) {
			modelError =
				new errors.RequiredFieldMissingError(
					'startDate parameter is required');
		...
		if (!(startDate instanceof Date)) {
			...
			app.log.trace('parsing startDate %s', startDate);
		}
		...
		data.offers.findAcceptedProposalsForNotification(
			startDate,
			options,
			function (err, offers) {
				if (err) {
					...
					modelError.startDate = startDate;
					modelError.options = options;

					return callback(modelError);
				}

				app.log.trace('find offers with accepted proposals in %s',
					countdown(startTime, new Date(), countdown.MILLISECONDS).toString());
