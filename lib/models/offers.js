...
module.exports = function (app, data, services, self) {
	...
	self.acceptOffer = function (postingId, offerId, acceptedTime, callback) {
		var
			modelError,
			startTime = new Date();
		...
		async.waterfall([
				...
				// ensure the offer only has one accepted time
				function (offer, done) {
					var isAccepted = false;

					offer.proposedTimes.some(function (time) {
						isAccepted = typeof time.acceptedAt !== 'undefined';
					...
					if (isAccepted) {
						return setImmediate(done,
							new errors.GeneralConflictError(
								'a proposedTime has already been accepted'));
					}
				...
				function (offer, done) {
					...
					offer.proposedTimes.some(function (time) {
						isFound =
							Number(time.when) === Number(new Date(acceptedTime.when)) &&
	...
	/**
	 * Allows for any accepted proposed times to be unaccepted
	 **/
	self.unacceptOffer = function (postingId, offerId, callback) {
		...
		async.waterfall([
				...
				function (offer, done) {
					offer.proposedTimes.forEach(function (time) {
						if (typeof time.acceptedAt !== 'undefined') {
							time.acceptedAt = undefined;
						}
					});
