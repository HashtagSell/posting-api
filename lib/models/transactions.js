...
module.exports = function (app, data, services, self) {
	...
	self.create = function (transaction, callback) {
		...
		async.waterfall([
				...
				/*
				// issue #90 - removal from search now requires a review
				// with isExchanged set to true
	...
	**/
	...
	self.update = function (transactionId, transaction, callback) {
		...
		async.waterfall([
				...
				/*,
				// related to issue #90 - no longer removing posting when transaction completes
				// removal of posting requires review with isExchanged set to true
